...
import math
import numpy as np
from vit_model import vit_b_cifar100 # Thay đổi import
...
# --- Argument Parser ---
parser = argparse.ArgumentParser(description='PyTorch CIFAR-100 ViT-B Training') # Thay đổi mô tả
...
parser.add_argument('--run_name', default='vit_b_cifar100', type=str, help='name for this run (used for checkpoint saving)') # Thay đổi run_name
...
# Sử dụng ViT-B từ vit_model.py, patch_size=4 cho CIFAR-100 (ảnh 32x32)
model = vit_b_cifar100(num_classes=100, image_size=32, patch_size=4)
...
if args.resume:
    ...
    if os.path.isfile(checkpoint_file):
        ...
        # Xử lý state_dict cho DataParallel
        is_dataparallel = isinstance(model, nn.DataParallel)
        model_is_currently_dataparallel = any(k.startswith('module.') for k in model.state_dict().keys())
        checkpoint_is_dataparallel = any(k.startswith('module.') for k in state_dict.keys())

        if checkpoint_is_dataparallel and not is_dataparallel:
            print("Loading DataParallel checkpoint into a non-DataParallel model.")
            new_state_dict = {k.replace('module.', ''): v for k, v in state_dict.items()}
            state_dict = new_state_dict
        elif not checkpoint_is_dataparallel and is_dataparallel:
            print("Loading non-DataParallel checkpoint into a DataParallel model.")
...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        # Accuracy calculation for mixup should be handled carefully.
        # For simplicity, we calculate accuracy based on the original targets if lam < 1, or predicted targets if lam = 1.
        # A more robust way might be to check against both targets_a and targets_b weighted by lam.
        # Here, we'll use the original targets for a simpler progress metric.
        # The reported accuracy during training with mixup might not be the most precise measure of true model performance on unmixed data.
        if lam == 1:
            correct += predicted.eq(targets_a).sum().item()
        else: # A common way to approximate during training with mixup
        ...
        if (batch_idx + 1) % 50 == 0 or batch_idx == len(trainloader) - 1:
            ...
            print(f'  Train Batch: [{batch_idx + 1}/{len(trainloader)}] ({progress:.0f}%) | Loss: {train_loss/(batch_idx+1):.3f} | Acc: {100.*correct/total:.3f}% ({int(correct)}/{total}) | LR: {current_lr:.6f}')
